...
public class MethodSearcher
{
    ...
    public Method getMethodsEventAnnotatedWith(String label) {
        for(Method method : getMethodsAnnotatedWith(Event.class)) {
    ...
    public Method getMethodsEventAnnotatedWith(String eventLabel, String eventRule) {
        for(Method method : getMethodsAnnotatedWith(Event.class)) {
            if(method.getAnnotation(Event.class).label().equals(eventLabel) &&
                    method.getAnnotation(Event.class).rule().equals(eventRule) )
                return method;
        }
        return null;
    }
